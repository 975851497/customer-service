...
from ..utils import count_total, format_date, format_datetime, money, offset_limit
            ...
            "list": [
                {
                    ...
                    "orderPayableAmount": money(row["order_payable_amount"]),
                    "orderPaidAmount": money(row["order_paid_amount"]),
...
def get_my_cohort_detail(
    ...
    if item is None:
        raise not_found("ORDER_ITEM_NOT_FOUND", "班次对应的订单明细不存在")
    return ok(
        {
            "cohortId": rel["cohort_id"],
            "cohortCode": item["cohort_code"],
            "cohortName": item["cohort_name"],
            "seriesId": item["series_id"],
            "seriesName": item["series_name"],
            "deliveryModeCode": item["delivery_mode"],
            "coverUrl": item["cover_url"],
            "studentId": item["student_id"],
            "orderId": item["order_id"],
            "orderNo": item["order_no"],
            "orderStatusCode": item["order_status"],
            "orderItemId": rel["order_item_id"],
            "orderItemStatusCode": item["order_item_status"],
            "itemName": item["item_name"],
            "unitPrice": money(item["unit_price"]),
            "discountAmount": money(item["discount_amount"]),
            "payableAmount": money(item["payable_amount"]),
            "orderPayableAmount": money(item["order_payable_amount"]),
            "orderPaidAmount": money(item["order_paid_amount"]),
