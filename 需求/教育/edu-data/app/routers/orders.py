...
def order_item_payload(row: dict[str, Any]) -> dict[str, object]:
    return {
        "orderItemId": row["id"],
...
def fetch_order_items_for_orders(
    ...
    for row in rows:
        result.setdefault(int(row["order_id"]), []).append(order_item_payload(row))
    return result
...
def list_orders(
    ...
    items_by_order = fetch_order_items_for_orders(order_ids, current_user_id)
    return ok(
        {
            "list": [
                order_list_payload(row, items_by_order.get(int(row["id"]), []))
                for row in rows
            ],
            "pageNo": page_no,
            "pageSize": page_size,
...
def get_order_detail(
    ...
    items = fetch_all(
        ...
        JOIN series_cohort AS cohort ON cohort.id = item.cohort_id
        JOIN series ON series.id = cohort.series_id
        WHERE item.order_id = %s AND item.user_id = %s
        ORDER BY item.id
        """,
        (order_id, current_user_id),
    )
    ...
    return ok(
        {
            ...
            "orderItems": [order_item_payload(item) for item in items],
            "paymentSummary": {
