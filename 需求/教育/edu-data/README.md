...
- 大多数接口必须传请求头 `X-User-Id`，值为 `sys_user.id`。
- 服务端会校验 `X-User-Id` 是合法数字，且用户存在并启用。
- 公共课程查询接口可不传 `X-User-Id`：
  - `GET /api/v1/series`
...
- 公共课程查询接口如果传了合法 `X-User-Id`，会额外返回当前用户和课程/班次的购买报名关联信息。
...
- 分页响应通常包含 `list`、`pageNo`、`pageSize`、`total`。
...
### 业务链路返回约定
为了方便客服系统和前端在任意对象上继续追溯完整业务链路，当前接口在课程、班次、订单、支付、退款、工单等接口中尽量补充上下游信息。

核心链路：

```text
series -> series_cohort -> order_item -> order -> payment_record
                                      -> refund_request -> service_ticket
student_cohort_rel -> series_cohort
student_cohort_rel -> order_item -> order
```

公共课程接口在带 `X-User-Id` 时返回用户关联信息：

- `GET /series` 和 `GET /series/{series_id}` 中的 `myEnrollments`：当前用户购买/报名到该课程系列下的班次列表。
- `GET /series/{series_id}/cohorts` 和 `GET /cohorts/{cohort_id}` 中的 `myEnrollment`：当前用户在该班次上的报名与订单关联；没有关联时为 `null`。

`myEnrollment` 或 `myEnrollments[]` 字段结构：

```json
{
  "cohortId": 30001,
  "cohortName": "2026 春季直播 1 班",
  "cohortCode": "COH202601LIVE01",
  "orderId": 70001,
  "orderNo": "ORD202601010001",
  "orderStatusCode": "paid",
  "orderItemId": 71001,
  "orderItemStatusCode": "paid",
  "itemName": "2026 春季直播 1 班",
  "payableAmount": 3999.0,
  "enrollStatusCode": "active",
  "enrollAt": "2026-01-01 10:00:00"
}
```

订单、支付、退款、工单中的 `order.orderItems[]` 或 `orderItems[]` 会补充课程与班次信息：
...
行为：用户不存在或停用时返回 `USER_NOT_FOUND_OR_DISABLED`。
...
{
  "studentId": 50123,
  "learnerIdentity": {"code": "in_school_student", "name": "在校生"},
  "learningGoal": {"code": "exam_preparation", "name": "升学备考"},
  "educationLevel": null,
  "grade": {"code": "senior_2", "name": "高中二年级"},
  "schoolName": "上海市实验中学",
  "profileNote": "目标冲刺重点高中"
}
```

行为：没有学员档案时返回 `STUDENT_PROFILE_NOT_FOUND`。
...
```json
{
  ...
  "recentLearningRecords": [
    {"type": "video", "title": "函数与参数", "occurredAt": "2026-01-01 20:00:00"}
  ]
}
```
...
- `keyword`：课程名称关键字。
- `categoryId`：课程分类 ID。
- `learningGoalCode`：学习目标编码。
- `deliveryModeCode`：授课方式，`online_recorded`、`online_live`、`offline_face_to_face`。
- `pageNo`、`pageSize`。
...
- 只返回 `sale_status = on_sale` 的课程系列。
- 评分按课程系列下所有班次评价聚合。
- 排序为 `avgScore desc, reviewCount desc, series.id desc`。
- 如果传 `keyword` 和合法 `X-User-Id`，写入 `series_search_log`。
- 如果传合法 `X-User-Id`，返回 `myEnrollments`，否则为空列表。
...
行为：课程不存在或未上架时返回 `SERIES_NOT_FOUND`；如果传合法 `X-User-Id`，返回 `myEnrollments`。
...
行为：只返回 `yn = 1` 且未满员班次；如果传合法 `X-User-Id`，每个班次返回 `myEnrollment`。
...
```json
{
  ...
  "myEnrollment": null,
  "modules": [
    {"cohortCourseId": 1, "moduleName": "基础模块", "stageNo": 1, "lessonCount": 12}
  ],
  "sessions": [
    {"sessionId": 1, "sessionTitle": "函数导论", "teachingDate": "2026-01-10", "startTime": "19:00:00", "endTime": "21:00:00"}
  ]
}
```

行为：班次不存在时返回 `COHORT_NOT_FOUND`；如果传合法 `X-User-Id`，返回 `myEnrollment`。
...
{
  "list": [
    {"seriesId": 9001, "seriesName": "高考数学冲刺班", "favoriteCreatedAt": "2026-01-01 10:00:00"}
  ],
  "pageNo": 1,
  "pageSize": 20,
  "total": 1
...
`favoriteSource` 可选值：`series_detail`、`search_result`、`recommendation`、`activity_page`。

返回字段：

```json
{"favoriteId": 1, "favorited": true, "created": true}
```

行为：课程不存在或未上架返回 `SERIES_NOT_FOUND`；来源非法返回 `INVALID_FAVORITE_SOURCE`。
...
```json
{
  ...
  "consultContent": "想了解开课时间",
  "sourceChannelId": 12
}
```

`consultChannel` 可选值：`phone`、`online_chat`、`wechat`、`offline_visit`。
...
行为：班次不存在返回 `COHORT_NOT_FOUND`；渠道不存在返回 `CHANNEL_NOT_FOUND`；没有可用顾问返回 `NO_CONSULTANT`。
...
{
  "list": [
    {"consultationId": 1, "cohortId": 30001, "consultChannel": "online_chat", "consultContent": "想了解开课时间", "consultedAt": "2026-01-01 10:00:00"}
  ],
  "pageNo": 1,
  "pageSize": 20,
  "total": 1
...
- `seriesId`：可选，按课程适用范围过滤。
- `categoryId`：可选，按分类适用范围过滤。
...
[
  {
    ...
    "validTo": "2026-02-01 23:59:59"
  }
]
```

行为：只返回有效期内、有库存、未超过当前用户领取上限、且满足适用范围的优惠券，最多 50 条。
...
```json
{"receiveId": 1, "receiveNo": "CR202601010001"}
```

行为：优惠券不存在返回 `COUPON_NOT_FOUND`；过期返回 `COUPON_EXPIRED`；库存不足返回 `COUPON_OUT_OF_STOCK`；超过个人领取上限返回 `COUPON_RECEIVE_LIMIT_EXCEEDED`。
...
{
  "list": [
    {"receiveId": 1, "couponName": "新人立减券", "receiveStatusCode": "unused", "receivedAt": "2026-01-01 10:00:00", "usedAt": null, "expiredAt": "2026-02-01 23:59:59"}
  ],
  "pageNo": 1,
  "pageSize": 20,
  "total": 1
...
```json
[
  {"itemId": 1, "cohortId": 30001, "cohortName": "2026 春季直播 1 班", "unitPrice": 3999.0, "addedAt": "2026-01-01 10:00:00"}
]
```

#### 6.2 `POST /api/v1/cart/items`
说明：加入购物车；已存在时复用原购物车项，若已移除则恢复。
...
```json
{"cohortId": 30001, "cartSource": "series_detail"}
```

`cartSource` 可选值：`series_detail`、`search_result`、`recommendation`、`activity_page`。
...
行为：班次不存在返回 `COHORT_NOT_FOUND`；来源非法返回 `INVALID_CART_SOURCE`。
...
```json
{"cohortId": 30001, "couponReceiveRecordId": null}
```
...
行为：优惠券不可用返回 `COUPON_UNAVAILABLE`；不适用返回 `COUPON_NOT_APPLICABLE`；未达门槛返回 `COUPON_THRESHOLD_NOT_MET`。
...
```json
{
  ...
  "payableAmount": 3999.0,
  "orderItems": [
    {"cohortId": 30001, "cohortName": "2026 春季直播 1 班", "seriesId": 9001, "seriesName": "高考数学冲刺班", "deliveryModeCode": "online_live", "itemName": "2026 春季直播 1 班", "payableAmount": 3999.0}
  ]
}
```
...
- `studentId` 必须属于当前用户。
- `orderSourceChannelId` 非空时必须是有效渠道。
- 如果当前用户在该班次有咨询记录，订单来源渠道必须匹配该班次咨询来源，否则返回 `ORDER_SOURCE_CHANNEL_MISMATCH`。

#### 7.3 `GET /api/v1/orders`
说明：查询当前用户订单列表，并补充订单明细、班次和课程信息。

请求头：`X-User-Id` 必填。

查询参数：

- `status`：可选，`pending`、`paid`、`completed`、`cancelled`、`partial_refunded`、`refunded`。
- `pageNo`、`pageSize`。
...
行为：列表按 `created_at desc, id desc` 排序。
...
```json
{
  ...
  "paidAmount": 3999.0,
  "orderItems": [],
  "paymentSummary": {"paymentStatusCode": "paid", "paidAt": "2026-01-01 10:05:00"},
  "refundSummary": {"refundRequestCount": 0, "refundAmount": 0}
}
```

行为：订单必须属于当前用户，否则返回 `ORDER_NOT_FOUND`。

#### 7.5 `POST /api/v1/orders/{order_id}/cancel`
说明：取消未支付订单，同时关闭该订单下待支付支付单，并取消订单明细。
...
```json
{"orderId": 70001, "orderStatusCode": "cancelled"}
```

行为：只允许取消 `pending` 订单，否则返回 `ORDER_NOT_CANCELABLE`。
...
`paymentChannelCode` 可选值：`wechat_pay`、`alipay`、`bank_card`。
...
```json
{
  ...
  "order": {"orderId": 70001, "orderNo": "ORD202601010001", "orderStatusCode": "pending", "studentId": 50123, "payableAmount": 3999.0, "paidAmount": null, "orderItems": []},
  "paymentChannelCode": "wechat_pay",
  "paymentStatusCode": "pending",
  "amount": 3999.0,
  "paymentParams": {"mockToken": "MOCK_PAY_TOKEN_80001"},
  "createdAt": "2026-01-01 10:00:00"
}
```

行为：支付渠道非法返回 `INVALID_PAYMENT_CHANNEL`；订单状态不允许创建支付单返回 `ORDER_NOT_CREATABLE_FOR_PAYMENT`。

#### 8.2 `GET /api/v1/orders/{order_id}/payments`
说明：查询订单下支付记录，包含订单、订单明细、班次和课程信息。
...
{
  "list": [
    {"paymentId": 80001, "paymentNo": "PAY202601010001", "orderId": 70001, "paymentChannelCode": "wechat_pay", "paymentStatusCode": "paid", "amount": 3999.0, "thirdPartyTradeNo": "TP_DEMO", "paidAt": "2026-01-01 10:05:00", "createdAt": "2026-01-01 10:00:00", "order": {}}
  ]
}
```

#### 8.3 `GET /api/v1/payments/{payment_id}`
说明：查询单笔支付详情，包含订单链路信息。
...
```json
{"paymentId": 80001, "paymentNo": "PAY202601010001", "orderId": 70001, "paymentChannelCode": "wechat_pay", "paymentStatusCode": "paid", "amount": 3999.0, "thirdPartyTradeNo": "TP_DEMO", "paidAt": "2026-01-01 10:05:00", "createdAt": "2026-01-01 10:00:00", "order": {}, "refundAmount": 0.0, "refundAt": null}
```
...
```json
{"paymentId": 80001, "paymentNo": "PAY202601010001", "orderId": 70001, "order": {}, "paymentStatusCode": "closed", "updatedAt": "2026-01-01 10:10:00"}
```

行为：`closeReason` 不能为空；支付单不存在或不属于当前用户返回 `PAYMENT_NOT_FOUND`。

#### 8.5 `POST /api/v1/payment-notifications/mock`
说明：模拟支付渠道异步回调。

请求头：`X-Demo-Payment-Signature` 必填，默认 `mock-payment-signature`。

请求体：

```json
{"paymentNo": "PAY202601010001", "orderId": 70001, "paymentChannelCode": "wechat_pay", "amount": 3999.0, "tradeStatus": "paid", "thirdPartyTradeNo": "TP_DEMO"}
```

返回字段：

```json
{"paymentId": 80001, "paymentStatusCode": "paid", "orderId": 70001, "orderStatusCode": "paid", "processed": true}
```

行为：

- 签名错误返回 `INVALID_PAYMENT_SIGNATURE`。
- `tradeStatus` 可选值：`paid`、`failed`、`closed`。
- 支付通知会校验订单 ID、支付渠道、金额与库中支付记录一致。
- 重复通知按幂等处理，返回 `processed = false`。
- `paid` 会同步更新 `payment_record`、`order`、`order_item`，并把已使用优惠券置为 `used`。
...
```json
{"refundType": "personal_reason", "refundReason": "时间冲突", "applyAmount": 100.0, "remark": "客服代提交"}
```

`refundType` 可选值：`personal_reason`、`course_unsatisfied`、`schedule_conflict`、`duplicate_purchase`。

返回字段：返回完整退款申请，并补充 `order` 链路。

```json
{"refundRequestId": 87001, "refundNo": "RF202601010001", "orderId": 70001, "orderItemId": 71001, "studentId": 50123, "refundStatusCode": "pending", "applyAmount": 100.0, "approvedAmount": null, "appliedAt": "2026-01-01 11:00:00", "approvedAt": null, "refundedAt": null, "order": {}}
```

行为：订单明细必须属于当前用户，且状态必须为 `paid` 或 `completed`；申请金额必须大于 0 且不超过明细应付金额；没有可退款支付记录返回 `PAYMENT_NOT_FOUND`；已有处理中申请返回 `REFUND_IN_PROGRESS`。
...
- `status`：可选，`pending`、`approved`、`rejected`、`refunded`。
- `pageNo`、`pageSize`。

返回字段：

```json
{"list": [{"refundRequestId": 87001, "refundNo": "RF202601010001", "orderId": 70001, "orderItemId": 71001, "studentId": 50123, "refundStatusCode": "pending", "applyAmount": 100.0, "approvedAmount": null, "appliedAt": "2026-01-01 11:00:00", "approvedAt": null, "refundedAt": null, "order": {}}], "pageNo": 1, "pageSize": 20, "total": 1}
```
...
返回字段：同 `GET /refund-requests` 的单条对象。

行为：退款申请不存在或不属于当前用户返回 `REFUND_REQUEST_NOT_FOUND`。

### 9. 报名关系与学习中心
#### 9.1 `GET /api/v1/me/cohorts`
说明：查询当前用户报名/在读/完成/取消/退费的班次，包含课程、订单和订单明细链路。

请求头：`X-User-Id` 必填。

查询参数：

- `status`：可选，`active`、`completed`、`cancelled`、`refunded`。
- `pageNo`、`pageSize`。
  ...
  "list": [
    {
      ...
      "discountAmount": 0.0,
      "payableAmount": 3999.0,
      "orderPayableAmount": 3999.0,
      "orderPaidAmount": 3999.0,
      "enrollStatusCode": "active",
      "enrollAt": "2026-01-01 10:00:00"
    }
  ],
...
说明：查询当前用户某班次报名详情，包含课程、订单和订单明细链路。

请求头：`X-User-Id` 必填。

返回字段：单个 `GET /me/cohorts` 列表对象，额外包含：

```json
{"serviceEndAt": null, "completedAt": "2026-03-20 20:00:00"}
```

行为：允许查询已取消/已退费关系；关系不存在返回 `COHORT_REL_NOT_FOUND`。
...
```json
[
  {"sessionId": 1, "sessionTitle": "函数导论", "teachingDate": "2026-01-10", "attendanceStatusCode": "present", "homeworkSubmitStatus": "submitted", "examAttemptStatus": "submitted"}
]
```

行为：只允许有效履约关系访问，即报名状态不能为 `cancelled` 或 `refunded`。
...
{
  "enrollStatusCode": "active",
  "attendance": {"totalSessions": 10, "presentCount": 8, "absentCount": 1},
  "video": {"totalVideos": 12, "completedVideos": 9, "watchedSeconds": 36000},
  "homework": {"totalHomeworks": 5, "submittedCount": 4, "correctedCount": 3, "expiredUnsubmittedCount": 1},
  "exam": {"totalExams": 2, "submittedCount": 1, "absentCount": 0}
}
```
...
```json
{"sessionId": 1, "sessionTitle": "函数导论", "teachingDate": "2026-01-10", "startTime": "19:00:00", "endTime": "21:00:00", "teachingStatusCode": "scheduled", "teachers": [{"staffId": 1, "teacherName": "王老师"}]}
```

行为：当前用户必须对课次所属班次有有效履约关系，否则返回 `SESSION_NOT_FOUND`。
...
```json
{"videoId": 1, "videoTitle": "函数导论录播", "durationSeconds": 3600, "resolutionLabel": "1080p", "fileUrl": "https://example.com/video.mp4"}
```

行为：当前用户必须对视频所属班次有有效履约关系，否则返回 `VIDEO_NOT_FOUND`。
...
```json
[{"chapterNo": 1, "chapterTitle": "导入", "startSecond": 0, "endSecond": 300}]
```
...
```json
{"list": [{"videoId": 1, "videoTitle": "函数导论录播", "lastPositionSeconds": 1200, "progressPercent": 66.5, "updatedAt": "2026-01-01 20:00:00"}], "pageNo": 1, "pageSize": 20, "total": 1}
```
...
- `status`：可选，`pending`、`submitted`、`expired_unsubmitted`。
- `cohortId`：可选。
- `dueBefore`：可选，格式 `YYYY-MM-DD HH:mm:ss`。
- `pageNo`、`pageSize`。

返回字段：

```json
{"list": [{"homeworkId": 1, "homeworkName": "二次函数专题作业", "cohortId": 30001, "sessionId": 1, "dueAt": "2026-01-12 23:59:59", "homeworkStatus": "submitted", "submissionId": 10, "submittedAt": "2026-01-11 20:00:00", "correctedAt": null, "totalScore": null}], "pageNo": 1, "pageSize": 20, "total": 1}
```
...
```json
{"homeworkId": 1, "homeworkName": "二次函数专题作业", "dueAt": "2026-01-12 23:59:59", "createdAt": "2026-01-10 10:00:00"}
```

行为：当前用户必须对作业所属班次有有效履约关系，否则返回 `HOMEWORK_NOT_FOUND`。
...
- `status`：可选，`submitted`、`expired_unsubmitted`。
- `pageNo`、`pageSize`。

返回字段：

```json
{"list": [{"submissionId": 10, "homeworkId": 1, "homeworkName": "二次函数专题作业", "submitStatus": "submitted", "correctionStatus": "corrected", "totalScore": 95.0, "submittedAt": "2026-01-11 20:00:00", "correctedAt": "2026-01-12 10:00:00"}], "pageNo": 1, "pageSize": 20, "total": 1}
```
...
- `status`：可选，`not_started`、`in_progress`、`submitted`、`absent`、`timeout`。
- `cohortId`：可选。
- `deadlineBefore`：可选，格式 `YYYY-MM-DD HH:mm:ss`。
- `pageNo`、`pageSize`。

返回字段：

```json
{"list": [{"examId": 1, "examName": "阶段测验", "cohortId": 30001, "sessionId": 1, "windowStartAt": "2026-01-15 19:00:00", "deadlineAt": "2026-01-16 23:59:59", "durationMinutes": 90, "attemptStatus": "not_started", "submissionId": null, "scoreValue": null}], "pageNo": 1, "pageSize": 20, "total": 1}
```
...
```json
{"examId": 1, "examName": "阶段测验", "totalScore": 100.0, "passScore": 60.0, "durationMinutes": 90, "windowStartAt": "2026-01-15 19:00:00", "deadlineAt": "2026-01-16 23:59:59"}
```

行为：当前用户必须对考试所属班次有有效履约关系，否则返回 `EXAM_NOT_FOUND`。
...
```json
{"list": [{"submissionId": 1, "examId": 1, "examName": "阶段测验", "attemptStatus": "submitted", "scoreValue": 88.0, "startAt": "2026-01-15 19:10:00", "submitAt": "2026-01-15 20:00:00"}], "pageNo": 1, "pageSize": 20, "total": 1}
```
...
```json
{"scoreOverall": 5, "scoreTeacher": 5, "scoreContent": 5, "scoreService": 5, "reviewTags": ["讲得清楚"], "reviewContent": "很好", "anonymousFlag": 0}
```

返回字段：

```json
{"reviewId": 1, "reviewNo": "RV202601010001"}
```

行为：分数必须在 1 到 5；`anonymousFlag` 只能为 `0` 或 `1`；当前用户必须有有效履约关系；同一用户同一班次只允许一条有效评价。

#### 11.2 `GET /api/v1/cohorts/{cohort_id}/reviews`
说明：查询班次评价列表。

请求头：`X-User-Id` 可选；`onlyMine=true` 时必填。
...
```json
{"list": [{"reviewId": 1, "scoreOverall": 5, "reviewTags": ["讲得清楚"], "reviewContent": "很好", "reviewedAt": "2026-01-01 10:00:00"}], "pageNo": 1, "pageSize": 20, "total": 1}
```

行为：`onlyMine=true` 但未传 `X-User-Id` 时返回 `MISSING_USER_ID`。

### 12. 工单与售后服务
#### 12.1 `GET /api/v1/service-tickets`
说明：查询当前用户工单列表，包含订单、订单明细、班次、课程和可选退款申请摘要。

请求头：`X-User-Id` 必填。

查询参数：

- `ticketType`：可选，`after_sales`、`complaint`、`refund`。
- `ticketStatus`：可选，`pending`、`in_progress`、`closed`。
- `pageNo`、`pageSize`。
...
```json
{
  "list": [
    {
      "ticketId": 99001,
      "ticketNo": "TK202601010001",
      "ticketType": "refund",
      "ticketStatus": "pending",
      "title": "课程退费工单",
      "openedAt": "2026-01-01 12:00:00",
      "order": {},
      "refundRequest": {"refundRequestId": 87001, "refundNo": "RF202601010001", "refundStatusCode": "pending", "applyAmount": 100.0}
    }
  ],
  "pageNo": 1,
  "pageSize": 20,
...
返回字段：返回创建后的工单摘要，包含 `order` 链路和可选 `refundRequest` 摘要。

```json
{"ticketId": 99001, "ticketNo": "TK202601010001", "ticketType": "refund", "ticketStatus": "pending", "title": "课程退费工单", "openedAt": "2026-01-01 12:00:00", "order": {}, "refundRequest": {}}
```

行为：

- `ticketType` 可选值：`after_sales`、`complaint`、`refund`。
- `priorityLevel` 可选值：`low`、`medium`、`high`、`urgent`。
- `ticketSource` 可选值：`user_app`、`customer_service`。
- 标题和内容不能为空。
- `ticketType = refund` 时必须传 `refundRequestId`，且退款申请必须属于当前用户并匹配订单明细。
- 非退款工单不能传 `refundRequestId`。

#### 12.3 `GET /api/v1/service-tickets/{ticket_id}`
说明：查询工单详情，包含订单链路和可选退款申请摘要。
...
```json
{"ticketId": 99001, "ticketNo": "TK202601010001", "ticketType": "refund", "ticketStatus": "in_progress", "title": "课程退费工单", "openedAt": "2026-01-01 12:00:00", "order": {}, "refundRequest": {}, "ticketContent": "用户申请课程退费", "refundRequestId": 87001, "closedAt": null}
```

行为：工单不存在或不属于当前用户返回 `TICKET_NOT_FOUND`。
...
```json
[
  {"followRecordId": 1, "followType": "refund_review", "followChannel": "phone", "followResult": "in_progress", "followContent": "已联系用户", "followedAt": "2026-01-01 13:00:00"}
]
```
...
行为：`scoreValue` 必须在 1 到 5；只有已关闭工单允许提交；同一工单只能提交一条有效满意度记录。
